...
- Sniper-only acquisition requires sniper aim enabled, the scope fully open,
  and the game's `m_DoingAimAssist + m_CurrentAimAssistTarget` trigger.
...
- Idle polling uses cached controller state. While active, only the pawn in
